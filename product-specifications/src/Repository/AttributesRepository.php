...
{
    private \Amiut\ProductSpecs\Repository\EntityCollectionFactory $factory;
    public function __construct(\Amiut\ProductSpecs\Repository\EntityCollectionFactory $factory)
    {
     ...
     */
    public function findCollection(array $args): \Amiut\ProductSpecs\Repository\EntityCollection
    {
    ...
    public function findByGroupSorted(int $groupId): array
    {
         ...
         */
        $orderedAttributesIds = array_map('absint', array_filter((array) get_term_meta($groupId, 'attributes', \true)));
        $terms = $this->findByGroup($groupId);
        usort($terms, static function (WP_Term $attribute1, WP_Term $attribute2) use ($orderedAttributesIds) {
            $position1 = array_search($attribute1->term_id, $orderedAttributesIds, \true);
            $position2 = array_search($attribute2->term_id, $orderedAttributesIds, \true);
    ...
    public function findByGroup(int $groupId): array
    {
        ...
        $terms = get_terms(['taxonomy' => Taxonomy\Attribute::KEY, 'hide_empty' => \false, 'meta_key' => 'attr_group', 'meta_value' => (string) $groupId]);
    ...
    private function prepareArguments(array $args): array
    {
        ...
        $preparedArguments = ['taxonomy' => Taxonomy\Attribute::KEY, 'hide_empty' => \false, 'orderby' => 'term_id', 'order' => 'DESC'];
        if (!empty($args['search_query'])) {
            $preparedArguments['search'] = $args['search_query'];
        }
        if (!empty($args['group_id'])) {
            $preparedArguments = $preparedArguments + ['meta_key' => 'attr_group', 'meta_value' => (string) $args['group_id']];
        }
