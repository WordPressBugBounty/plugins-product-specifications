...
final class AttributeGroupController
{
    ...
    {
        if (empty($data['name'])) {
            wp_send_json_error(['result' => 'error', 'message' => esc_html__('Please enter a group name', 'product-specifications'), 'where' => ['input#group_name'], 'action' => 'add']);
        }
        $check = wp_insert_term($data['name'], 'spec-group', ['description' => $data['description'], 'slug' => $data['slug']]);
        if (is_wp_error($check)) {
            wp_send_json_error(['result' => 'error', 'message' => esc_html__('Something went wrong', 'product-specifications'), 'action' => 'add']);
            return;
        }
        wp_send_json_success(['result' => 'success', 'message' => esc_html__('Group created successfully', 'product-specifications'), 'action' => 'add']);
    }
    private function processEdit(array $data): void
    {
        ...
        if (!is_int($data['id']) || $data['id'] < 1) {
            wp_send_json_error(['result' => 'error', 'message' => esc_html__('Invalid Group', 'product-specifications'), 'where' => ['input#group_id'], 'action' => 'add']);
        }
        $check = wp_update_term($data['id'], 'spec-group', ['name' => $data['name'], 'description' => $data['description'], 'slug' => $data['slug']]);
        if (is_wp_error($check)) {
            wp_send_json_error(['result' => 'error', 'message' => esc_html__('Something went wrong', 'product-specifications'), 'action' => 'add']);
            return;
        }
        wp_send_json_success(['result' => 'success', 'message' => esc_html__('Group edited successfully', 'product-specifications'), 'action' => 'add']);
    }
    private function processDelete(): void
    {
        ...
        if ($check instanceof \WP_Error) {
            wp_send_json_error(['result' => 'error', 'message' => esc_html__('Could not delete group(s)', 'product-specifications'), 'action' => 'delete']);
            return;
        }
        wp_send_json_success(['result' => 'success', 'message' => esc_html__('Group(s) deleted successfully', 'product-specifications'), 'action' => 'delete']);
    }
