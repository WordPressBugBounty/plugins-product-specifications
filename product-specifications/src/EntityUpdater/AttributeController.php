...
final class AttributeController
{
    ...
    private function processAdd(array $data): void
    {
        try {
            $this->validate($data);
            $check = wp_insert_term($data['name'], Taxonomy\Attribute::KEY, ['description' => $data['description'], 'slug' => $data['slug']]);
            if ($check instanceof \WP_Error) {
                wp_send_json_error(['result' => 'error', 'message' => esc_html(sprintf(
            ...
            do_action_deprecated(self::ACTION_ATTRIBUTES_MODIFIED, ['action' => self::ACTION_ADD, 'ids' => [$check['term_id']]], '0.8.0', self::ACTION_ATTRIBUTES_ADDED);
            do_action(self::ACTION_ATTRIBUTES_ADDED, $check['term_id']);
            wp_send_json_success(['result' => 'success', 'message' => esc_html__('Attribute added successfully', 'product-specifications'), 'action' => self::ACTION_ADD]);
        } catch (\Amiut\ProductSpecs\EntityUpdater\InvalidEntityPropertyException $exception) {
            wp_send_json_error(['result' => 'error', 'message' => $exception->getMessage(), 'where' => [$exception->where()], 'action' => self::ACTION_ADD]);
        }
    }
    // phpcs:ignore Inpsyde.CodeQuality.FunctionLength.TooLong
    private function processEdit(array $data): void
    {
        try {
            ...
            if ($attributeId < 1) {
                wp_send_json_error(['result' => 'error', 'message' => 'invalid request', 'where' => [], 'action' => self::ACTION_EDIT]);
            }
            $check = wp_update_term($attributeId, Taxonomy\Attribute::KEY, ['name' => $data['name'], 'description' => $data['description'], 'slug' => $data['slug']]);
            if ($check instanceof \WP_Error) {
                wp_send_json_error(['result' => 'error', 'message' => esc_html__('Unable to update the attribute', 'product-specifications'), 'where' => [], 'action' => self::ACTION_EDIT]);
            }
            $this->updateTermMetas($data, $check['term_id']);
            do_action_deprecated(self::ACTION_ATTRIBUTES_MODIFIED, ['action' => self::ACTION_EDIT, 'ids' => [$check['term_id']]], '0.8.0', self::ACTION_ATTRIBUTES_UPDATED);
            do_action(self::ACTION_ATTRIBUTES_UPDATED, $check['term_id']);
            wp_send_json_success(['result' => 'success', 'message' => esc_html__('Attribute updated successfully', 'product-specifications'), 'action' => self::ACTION_EDIT]);
        } catch (\Amiut\ProductSpecs\EntityUpdater\InvalidEntityPropertyException $exception) {
            wp_send_json_error(['result' => 'error', 'message' => $exception->getMessage(), 'where' => [$exception->where()], 'action' => self::ACTION_EDIT]);
        }
    }
    private function processDelete(): void
    {
        $ids = array_map('absint', (array) filter_input(\INPUT_POST, 'id', \FILTER_SANITIZE_NUMBER_INT, \FILTER_FORCE_ARRAY));
        $check = null;
        foreach ($ids as $id) {
            $groupId = (int) get_term_meta($id, 'attr_group', \true);
            /**
        ...
        do_action_deprecated(self::ACTION_ATTRIBUTES_MODIFIED, ['action' => self::ACTION_EDIT, 'ids' => $ids], '0.8.0', self::ACTION_ATTRIBUTES_DELETED);
        wp_send_json_success(['result' => 'success', 'message' => esc_html__('Attribute(s) deleted successfully', 'product-specifications'), 'action' => 'delete']);
    }
    /**
    ...
    {
        if (empty($data['name'])) {
            throw new \Amiut\ProductSpecs\EntityUpdater\InvalidEntityPropertyException(esc_html__('Please enter an attribute name', 'product-specifications'), 'input#attr_name');
        }
        if (empty($data['group'])) {
            throw new \Amiut\ProductSpecs\EntityUpdater\InvalidEntityPropertyException(esc_html__('Please select a group', 'product-specifications'), 'input#attr_group');
        }
        if (empty($data['type'])) {
            throw new \Amiut\ProductSpecs\EntityUpdater\InvalidEntityPropertyException(esc_html__('Please select a field type', 'product-specifications'), 'input#attr_type');
        }
        if (($data['type'] === 'select' || $data['type'] === 'radio') && empty($data['values'])) {
            throw new \Amiut\ProductSpecs\EntityUpdater\InvalidEntityPropertyException(esc_html__('Please set some values for attribute', 'product-specifications'), 'input#attr_values');
        }
    }
    private function updateTermMetas(array $data, int $termId): void
    {
        $termMetaMapping = ['attr_group' => $data['group'], 'attr_type' => $data['type'], 'attr_values' => array_filter(array_map('trim', $data['values'])), 'attr_default' => rawurldecode((string) $data['default'])];
    ...
    {
        return in_array($action, [self::ACTION_ADD, self::ACTION_EDIT, self::ACTION_DELETE], \true) ? $action : 'add';
    }
}
